<script lang="ts">
	...
	let { questions, title, description, difficulty, backRoute, backParams, backLabel } = $props<{
		questions: Question[];
		title: string;
		description: string;
		difficulty?: string;
	...
	let isFinished = $derived(questions.length > 0 && currentIndex >= questions.length);
	let totalDots = $derived(questions.length);
...
<div class="mx-auto w-full max-w-2xl">
	{#if questions.length === 0}
		...
		>
			...
			<p class="mb-5 text-sm text-slate-500">
				Latihan ini masih disiapkan. Nantikan pembahasannya di pembaruan berikutnya.
			</p>
			<a
				href={resolve(backRoute, backParams)}
	...
	{:else if isFinished}
		<div class="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm">
			<p class="mb-2 text-sm text-slate-500">{title} selesai</p>
